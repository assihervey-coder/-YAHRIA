...
// (toutes PROMOTED).
//
//   bun scripts/pta-repair-measure-it12.ts slot N        — exécute UN slot
//   bun scripts/pta-repair-measure-it12.ts reconstruct N UID — verdict DB
//   bun scripts/pta-repair-measure-it12.ts finalize      — agrège + scelle
//
...
async function preflight(): Promise<void> {
  log('PRÉ-VOL — serveur, fabric, armement few-shot EVO-000027 + boucle EVO-000028 + fidélité EVO-000029 + budget par porte EVO-000030 + fermeture EVO-000031 + registres/fidélité-boot EVO-000032');
  ...
  log(`PRÉ-VOL OK — order=${JSON.stringify(fabric.order)} zai=CLOSED few-shot=ARMÉ boucle=ARMÉE fidélité=ARMÉE budget-par-porte+pydantic-v2=ARMÉS fermeture-dépendances=ARMÉE registres=ARMÉS fidélité-boot=ARMÉE (EVO-000029 ${evo29.state} par ${evo29.decidedBy}, EVO-000030/31/32 PROMOTED)`);
}
...
async function collectVerdict(slot: number): Promise<SlotVerdict> {
  ...
  for (;;) {
    ...
    const name = `PTA-002 it.12 (contrats de registre + fidélité boot EVO-000032) run ${slot}/3${suffix} — Hub Mobile Money, triple porte + boucle bornée`;
    ...
    if (run.state === 'TIMEOUT' || infra) {
      ...
      await captureAndPersist({
        category: 'INCIDENT', criticality: 'HIGH', actorType: 'SYSTEM', actorId: 'pta-repair-measure-it12',
        claim: `PTA-002 it.12 slot ${slot} : run ${runUid} classé ${cls} — exclu des métriques d'apprentissage (INV-210), rejeu prévu`,
        payload: { runUid, slot, classification: cls, error: (run.error ?? '').slice(0, 500) },
        traceId: `TRACE-PTA002-IT12-S${slot}`,
      });
...
const VERDICTS_FILE = 'scripts/repair-verdicts-it12.jsonl';
...
async function main(): Promise<void> {
  ...
  if (cmd === 'slot') {
    ...
    console.log('\n═══ RÉSULTATS PTA-002 it.12 — CONTRATS DE REGISTRE + FIDÉLITÉ BOOT (EVO-000032) ═══');
    ...
    await captureAndPersist({
      ...
      traceId: 'TRACE-PTA002-IT12',
    });

    await Bun.write('scripts/repair-measure-result-it12.json', JSON.stringify({
