#!/usr/bin/env bun
// ═══════════════════════════════════════════════════════════════════
// YAHRIA — MESURE PTA-002 ITÉRATION 11 — FERMETURE DES DÉPENDANCES
// (EVO-000031 PROMOTED — décision HUMAN:reviewer, INV-227) enrichit la
// boucle EVO-000028 + fidélité EVO-000029 + budget par porte/pydantic v2
// EVO-000030 + few-shot EVO-000027 (toutes PROMOTED).
//
//   bun scripts/pta-repair-measure-it11.ts slot N        — exécute UN slot
//   bun scripts/pta-repair-measure-it11.ts reconstruct N UID — verdict DB
//   bun scripts/pta-repair-measure-it11.ts finalize      — agrège + scelle
//
...
// FERMETURE DES DÉPENDANCES (imports réels de la cible d'abord, 5
// frères / 3600 car. — DC1/DC2) + BF-DC1 (imports relatifs résolus).
//
...
//                                   (mécanismes prouvés en live) ; fabric
//                                   DÉGRADÉE — AC2 non comparable ; causes
//                                   restantes PROUVÉES par reproduction :
...
import { isRunRepairActive, isDependencyClosureActive } from '../src/lib/yahria/run-repair-loop';
...
async function preflight(): Promise<void> {
  log('PRÉ-VOL — serveur, fabric, armement few-shot EVO-000027 + boucle EVO-000028 + fidélité EVO-000029 + budget par porte EVO-000030 + fermeture EVO-000031');
  ...
  log(`PRÉ-VOL OK — order=${JSON.stringify(fabric.order)} zai=CLOSED few-shot=ARMÉ boucle=ARMÉE fidélité=ARMÉE budget-par-porte+pydantic-v2=ARMÉS fermeture-dépendances=ARMÉE (EVO-000029 ${evo29.state} par ${evo29.decidedBy}, EVO-000030 PROMOTED, EVO-000031 PROMOTED)`);
}
...
async function collectVerdict(slot: number): Promise<SlotVerdict> {
  ...
  for (;;) {
    ...
    const name = `PTA-002 it.11 (fermeture des dépendances EVO-000031) run ${slot}/3${suffix} — Hub Mobile Money, triple porte + boucle bornée`;
    ...
    if (run.state === 'TIMEOUT' || infra) {
      ...
      await captureAndPersist({
        category: 'INCIDENT', criticality: 'HIGH', actorType: 'SYSTEM', actorId: 'pta-repair-measure-it11',
        claim: `PTA-002 it.11 slot ${slot} : run ${runUid} classé ${cls} — exclu des métriques d'apprentissage (INV-210), rejeu prévu`,
        payload: { runUid, slot, classification: cls, error: (run.error ?? '').slice(0, 500) },
        traceId: `TRACE-PTA002-IT10-S${slot}`,
      });
...
const VERDICTS_FILE = 'scripts/repair-verdicts-it11.jsonl';
...
async function main(): Promise<void> {
  ...
  if (cmd === 'slot') {
    ...
    console.log('\n═══ RÉSULTATS PTA-002 it.11 — FERMETURE DES DÉPENDANCES (EVO-000031) ═══');
    ...
    console.log(`  Runs SEALED total          : ${sealedTotal}/${counted.length} (baselines it.7 : 0/3 · it.8 : 0/3 · it.9 : 0/3)`);
    ...
    console.log(`  Taux première passe fichiers : ${(aggregateRate * 100).toFixed(1)} % (it.7 : 97,4 % · it.8 : 53,8 % · it.9 : 84,6 % — réparation ≠ première passe)`);
    ...
    await captureAndPersist({
      category: 'ARTIFACT', criticality: 'HIGH', actorType: 'AGENT', actorId: 'pta-repair-measure-it11',
      claim: `PTA-002 itération 10 (budget de réparation PAR PORTE ≤2 cycles/run + contrat pydantic v2 + contrat comportemental EVO-000030, boucle EVO-000028, fidélité EVO-000029, few-shot EVO-000027 actifs) : ${sealedTotal}/${counted.length} runs SEALED (cycle 0 : ${sealedCycle0}, cycle ≤1 : ${sealedCycle1}, cycle ≤2 : ${sealedCycle2}), ${repairsEngaged} run(s) avec boucle engagée, taux première passe ${(aggregateRate * 100).toFixed(1)} % sous triple porte — AC1 ${ac1 ? 'PASS' : 'FAIL'}, AC2 ${ac2 ? 'PASS' : 'FAIL'}, AC3 ${ac3 ? 'PASS' : 'FAIL'}, AC4 ${ac4 ? 'PASS' : 'FAIL'} (baselines it.7 : 0/3, 97,4 % · it.8 : 0/3, 53,8 % · it.9 : 0/3, 84,6 %)`,
      payload: {
      ...
      traceId: 'TRACE-PTA002-IT10',
    });

    await Bun.write('scripts/repair-measure-result-it11.json', JSON.stringify({
