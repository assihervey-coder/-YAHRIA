#!/usr/bin/env bun
// ═══════════════════════════════════════════════════════════════════
// YAHRIA — MESURE PTA-002 ITÉRATION 10 — BUDGET DE RÉPARATION PAR PORTE
// + CONTRAT PYDANTIC V2 (EVO-000030 PROMOTED — décision HUMAN:reviewer,
// INV-227) enrichit la boucle EVO-000028 + fidélité EVO-000029 +
// few-shot EVO-000027 (toutes PROMOTED).
//
//   bun scripts/pta-repair-measure-it10.ts slot N        — exécute UN slot
//   bun scripts/pta-repair-measure-it10.ts reconstruct N UID — verdict DB
//   bun scripts/pta-repair-measure-it10.ts finalize      — agrège + scelle
//
...
// ≤2 cycles/run) + contrat pydantic v2 (model_validate / from_attributes
// / model_dump) + contrat comportemental (le pytest EST le contrat).
//
...
// INCIDENT EV-POLICY-000001) — comparaison it.7 ↔ it.8 ↔ it.9 ↔ it.10
...
async function preflight(): Promise<void> {
  log('PRÉ-VOL — serveur, fabric, armement few-shot EVO-000027 + boucle EVO-000028 + fidélité EVO-000029 + budget par porte EVO-000030');
  ...
  log(`PRÉ-VOL OK — order=${JSON.stringify(fabric.order)} zai=CLOSED few-shot=ARMÉ boucle=ARMÉE fidélité=ARMÉE budget-par-porte+pydantic-v2=ARMÉS (EVO-000029 ${evo29.state} par ${evo29.decidedBy}, EVO-000030 PROMOTED)`);
}
...
async function collectVerdict(slot: number): Promise<SlotVerdict> {
  ...
  for (;;) {
    ...
    const name = `PTA-002 it.10 (budget par porte + pydantic v2 EVO-000030) run ${slot}/3${suffix} — Hub Mobile Money, triple porte + boucle bornée`;
    ...
    if (run.state === 'TIMEOUT' || infra) {
      ...
      await captureAndPersist({
        category: 'INCIDENT', criticality: 'HIGH', actorType: 'SYSTEM', actorId: 'pta-repair-measure-it10',
        claim: `PTA-002 it.10 slot ${slot} : run ${runUid} classé ${cls} — exclu des métriques d'apprentissage (INV-210), rejeu prévu`,
...
const VERDICTS_FILE = 'scripts/repair-verdicts-it10.jsonl';
...
async function main(): Promise<void> {
  ...
  if (cmd === 'slot') {
    ...
    console.log('\n═══ RÉSULTATS PTA-002 it.10 — BUDGET PAR PORTE + PYDANTIC V2 (EVO-000030) ═══');
    ...
    await captureAndPersist({
      category: 'ARTIFACT', criticality: 'HIGH', actorType: 'AGENT', actorId: 'pta-repair-measure-it10',
    ...
    await Bun.write('scripts/repair-measure-result-it10.json', JSON.stringify({
