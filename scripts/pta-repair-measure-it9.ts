#!/usr/bin/env bun
// ═══════════════════════════════════════════════════════════════════
// YAHRIA — MESURE PTA-002 ITÉRATION 9 — FIDÉLITÉ DES ENTRÉES + CONTRATS
// INTER-FICHIERS (EVO-000029 PROMOTED — décision HUMAN:reviewer, INV-227)
// enrichit la boucle de réparation EVO-000028 (toujours PROMOTED).
//
//   bun scripts/pta-repair-measure-it9.ts slot N        — exécute UN slot
//   bun scripts/pta-repair-measure-it9.ts reconstruct N UID — verdict DB
//   bun scripts/pta-repair-measure-it9.ts finalize      — agrège + scelle
//
// Protocole : 3 runs sur le MÊME brief canonique Hub Mobile Money (13
// fichiers, PYTHON), TRIPLE PORTE + few-shot EVO-000027 + boucle EVO-000028
// ENRICHIE EVO-000029 (RC1 stderr dès spawn, RC2 traceback pytest, RC3
// contrats frères, verdict 2000 car.).
//
// Baselines documentées :
//   it.7 (few-shot seul)         : 0/3 SEALED, 97,4 % première passe.
//   it.8 (boucle EVO-000028)     : 0/3 SEALED (0 cycle 0, 0 cycle ≤1),
//   3/3 boucles engagées, 53,8 % — causes racines EV-ARTIFACT-000212 :
//   entrées appauvries (traceback perdu, résumé sans cause) + dérive
//   inter-fichiers (PaymentInitiate importé ici, défini nulle part).
// Critères it.9 :
...
async function preflight(): Promise<void> {
  log('PRÉ-VOL — serveur, fabric, armement few-shot EVO-000027 + réparation EVO-000028 + fidélité EVO-000029');
  ...
  log(`PRÉ-VOL OK — order=${JSON.stringify(fabric.order)} zai=CLOSED few-shot=ARMÉ réparation=ARMÉE fidélité+contrats=ARMÉS (EVO-000029 PROMOTED par ${evo29.decidedBy})`);
}
...
async function collectVerdict(slot: number): Promise<SlotVerdict> {
  ...
  for (;;) {
    ...
    const name = `PTA-002 it.9 (fidélité + contrats EVO-000029) run ${slot}/3${suffix} — Hub Mobile Money, triple porte + boucle bornée`;
    ...
    if (run.state === 'TIMEOUT' || infra) {
      ...
      await captureAndPersist({
        category: 'INCIDENT', criticality: 'HIGH', actorType: 'SYSTEM', actorId: 'pta-repair-measure-it9',
        claim: `PTA-002 it.9 slot ${slot} : run ${runUid} classé ${cls} — exclu des métriques d'apprentissage (INV-210), rejeu prévu`,
        payload: { runUid, slot, classification: cls, error: (run.error ?? '').slice(0, 500) },
        traceId: `TRACE-PTA002-IT9-S${slot}`,
      });
...
const VERDICTS_FILE = 'scripts/repair-verdicts-it9.jsonl';
...
async function main(): Promise<void> {
  ...
  if (cmd === 'slot') {
    ...
    console.log('\n═══ RÉSULTATS PTA-002 it.9 — FIDÉLITÉ + CONTRATS EVO-000029 (boucle EVO-000028 enrichie) ═══');
    ...
    await captureAndPersist({
      category: 'ARTIFACT', criticality: 'HIGH', actorType: 'AGENT', actorId: 'pta-repair-measure-it9',
      claim: `PTA-002 itération 9 (fidélité des entrées + contrats inter-fichiers EVO-000029, boucle EVO-000028, few-shot EVO-000027 actifs) : ${sealedTotal}/${counted.length} runs SEALED (cycle 0 : ${sealedCycle0}, cycle ≤1 scellé : ${sealedRepair}), ${repairsEngaged} boucle(s) engagée(s), taux première passe ${(aggregateRate * 100).toFixed(1)} % sous triple porte — AC1 ${ac1 ? 'PASS' : 'FAIL'}, AC2 ${ac2 ? 'PASS' : 'FAIL'}, AC3 ${ac3 ? 'PASS' : 'FAIL'}, AC4 ${ac4 ? 'PASS' : 'FAIL'} (baselines it.7 : 0/3, 97,4 % · it.8 : 0/3, 53,8 %, 3 causes racines EV-ARTIFACT-000212)`,
      payload: {
        fidelity: 'EVO-000029 PROMOTED (HUMAN:reviewer) — RC1 stderr dès spawn, RC2 traceback pytest, RC3 contrats frères',
        repairLoop: 'EVO-000028 PROMOTED (HUMAN:reviewer)', fewShot: 'EVO-000027 PROMOTED (HUMAN:reviewer)',
        baselines: { it7: { runs: 'RUN-000010/11/12', sealedFirstPass: '0/3', firstPassRate: '97,4 %' }, it8: { runs: 'RUN-000013..20', sealed: '0/3', firstPassRate: '53,8 %', rootCauses: 'EV-ARTIFACT-000212' } },
      ...
      traceId: 'TRACE-PTA002-IT9',
    });

    await Bun.write('scripts/repair-measure-result-it9.json', JSON.stringify({
