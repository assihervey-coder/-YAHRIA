                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {na && (
                    <Button size="sm" className="h-7 text-[11px] bg-teal-600 hover:bg-teal-500 text-white"
                      onClick={() => {
                        const reason = ['approve', 'reject'].includes(na.action) ? window.prompt(`Raison gouvernée (≥ 10 caractères) — identité décisionnaire (ex : HUMAN ou AGENT:reviewer) :`) || '' : '';
                        if (['approve', 'reject'].includes(na.action)) {
                          if (reason.length < 10) return;
                          const idLine = reason.split('\n')[0];
                          act({ action: na.action, proposalUid: p.proposalUid, actor: idLine.startsWith('AGENT:') ? { type: 'AGENT', id: idLine.slice(6) } : { type: 'HUMAN', id: 'reviewer' }, reason }, na.action);
                        } else if (na.action === 'promote') {
                          act({ action: 'promote', proposalUid: p.proposalUid, actor: { type: 'HUMAN', id: 'release' }, experiment: { note: 'expérimentation documentée dans la proposition' } }, 'promote');
                        } else {
